...
export default function DoctorPatientPrescriptionsPage() {
  ...
  useEffect(() => {
    ...
    const fetchPrescriptions = async () => {
      try {
        const response = await fetch(`${apiBase}/api/doctor/patients/${patientId}/prescriptions`, {
        ...
        if (response.ok) {
          ...
          if (data.prescriptions && data.prescriptions.length > 0) {
            setPatientName(data.prescriptions[0].patient_name)
          }
        } else if (response.status === 404) {
          setError('Patient not found or you do not have access')
        } else {
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-6 py-8">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <Link href="/doctor/patients" className="text-cyan-400 hover:text-cyan-300 text-sm mb-4 inline-block">
            ← Back to Patients
          </Link>
          <div className="flex items-center justify-between">
            <div>
              ...
              {patientName && (
                <p className="text-slate-400">Prescriptions for <span className="font-semibold text-cyan-300">{patientName}</span></p>
              )}
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-slate-600 bg-slate-900/30 p-12 text-center">
            ...
            <h3 className="text-xl font-semibold text-slate-300 mb-2">No prescriptions found</h3>
            <p className="text-slate-400 mb-6">This patient hasn't uploaded any prescriptions yet</p>
            <Link href="/doctor/patients">
              <button className="px-6 py-3 rounded-full bg-cyan-500 text-slate-950 font-semibold hover:bg-cyan-400 transition">
                Back to Patients
              </button>
